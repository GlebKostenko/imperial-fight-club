...
async def apply_seed(db):
    ...
    # Миграция FAQ: обновляет старый базовый список один раз, но не перетирает будущие ручные правки.
    await db.settings.update_one(
        {"key": "site", "$or": [{"value.faqVersion": {"$exists": False}}, {"value.faqVersion": {"$lt": SITE_SETTINGS["faqVersion"]}}]},
        {"$set": {"value.faq": SITE_SETTINGS["faq"], "value.faqVersion": SITE_SETTINGS["faqVersion"], "updatedAt": now()}},
    )
